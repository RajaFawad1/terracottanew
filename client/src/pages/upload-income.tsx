...
export default function UploadIncome() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await apiRequest("POST", "/api/income-entries/upload", formData);
      ...
      if (result.success > 0) {
        toast({
          ...
          description: `Successfully uploaded ${result.success} income entries`,
        });
  ...
  const downloadTemplate = () => {
    const templateData = [
      ["Date", "First Name", "Last Name", "Payment Method", "Total Amount", "Tax Percentage", "Description"],
      ["2024-01-15", "Admin", "User", "Bank Transfer", "1000.00", "10", "Sample income entry"],
    ];

    const ws = XLSX.utils.aoa_to_sheet(templateData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Income Data");
    XLSX.writeFile(wb, "income_template.xlsx");
  };

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Upload Income Data</h1>
        <p className="text-muted-foreground mt-2">
          Upload income entries from an Excel file. Download the template below to see the required format.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select an Excel file to upload income entries</CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            {uploadResult.success > 0 && (
              <Alert>
                ...
                <AlertDescription>
                  {uploadResult.success} income entries have been successfully uploaded and are now visible in the Transactions page.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
